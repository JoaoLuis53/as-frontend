...
import { EventTabsGroups } from "../groups/EventTabsGroups";
    ...
    return (
        <div>
            <div className="flex text-center border-b border-gray-500 cursor-pointer">
                ...
                <div onClick={() => setTab('people')} className={`flex-1 p-3 hover:bg-gray-600 ${tab === 'people' ? 'bg-gray-600' : ''}`}>Pessoas</div>
            </div>
            <div>
                {tab === 'info' && <EventTabInfo event={event} refreshAction={refreshAction} />}
                {tab === 'groups' && <EventTabsGroups eventId={event.id} />}
